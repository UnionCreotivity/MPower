<template>
  <div class="calculator-view-root">
            ...
            <div class="loan-view-input-container">
              <div class="loan-input-item floor-select">
                ...
                <div class="loan-input">
                  <select v-model="loanInputModel.householdType" name="household-type">
    ...
    <div class="print-box" v-if="showPrintBox">
      <div class="logo-box">
      ...
      <div class="top-box">
        <div class="info-item">
          ...
          <div class="info">
            {{ loanInputModel.householdType }} 戶 / {{ loanInputModel.floor }} 樓
          </div>
        </div>
        <div class="info-item">
          <div class="title">戶別價格</div>
          <div class="row">
            <div class="info">{{ loanInputModel.householdPrice }}</div>
            <div class="text">萬元</div>
          </div>
        </div>
        <div class="info-item">
          <div class="title">車　　位</div>
          <div class="info">{{ loanInputModel.carAmount }}</div>
        </div>
        <div class="info-item">
          <div class="title">車位價格</div>
          <div class="row">
            <div class="info">{{ loanInputModel.carPrice }}</div>
            <div class="text">萬元</div>
          </div>
        </div>
        <div class="info-item">
          ...
          <div class="row">
            <div class="info">{{ loanInputModel.total }}</div>
            <div class="text">萬元</div>
          </div>
        </div>
      </div>
      ...
      <div class="hint-box">
        <div>
          ...
          2.暫收款內容包含：每戶新台幣貳拾伍萬元(多退少補)。包含所有權移轉登記規費、印花稅、代書費、代辦續費、貸款保險費...等<br />
          3.本付款表銀行貸款之成數、利率，需視客戶當時信用狀況而定。<br />
          4.買方可於辦理銀行貸款時，將「契稅申報」及「交屋款」期款併入申請貸款，若申貸金額核准，則「契稅申報」期款及「銀行貸款」期款於所有權移轉登記完成日一次撥付賣方指定帳戶。<br />
        </div>
      </div>
...
const handlePrint = () => {
  ...
  // 下一個 tick 等 DOM 更新再呼叫列印
  setTimeout(() => {
    window.print()
    showPrintBox.value = false // 列印完再關閉
  }, 100)
}
</script>
